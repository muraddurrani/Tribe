...
function index({navigation, route}) {
  ...
  const onSend = useCallback((messages = []) => {
    ...
    firestore().collection('Providers').doc(user._id).collection('Messages').doc(msgData.clientID).update({ lastUpdated: firebase.firestore.FieldValue.serverTimestamp()})
    firestore().collection('Clients').doc(msgData.clientID).collection('Messages').doc(user._id).update({ lastUpdated: firebase.firestore.FieldValue.serverTimestamp()})
  }, [])
    ...
    <KeyboardGradientView>
      ...
      <View style = {styles.card}>
        <GiftedChat
          ...
          user={ userData.profilePhoto ? { _id: user.uid, name: userData.fullName, avatar: userData.profilePhoto} : { _id: user.uid, name: userData.fullName }}
        />
      </View>
    </KeyboardGradientView>
  )
