...
function index({ navigation }) {
  ...
  const fetchData = async () => {
    const dataDoc = await firestore().collection('ProviderAttributes').doc('5').get()
  ...
  const submitChoices = () => {
    const ansCollection = firestore().collection('SearchProviders').doc('5').collection('Answers')
  ...
  return (
    <ScreenView style = {styles.container}>
      ...
      <ScreenView style = {styles.card}>
        <Text style = {styles.description} h4>What age groups do you support?</Text>
