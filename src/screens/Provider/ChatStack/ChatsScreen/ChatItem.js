...
function ChatItem({item}) {
  ...
  return (
    <ListItem
      ...
      onPress = {() => navigation.navigate('Message', {'ID': item.ID, 'msgRef': item.msgRef, fullName: data.parentFullName, profilePhoto: data.profilePhoto})}
    >
      <ProfileIcon photo = {data.profilePhoto} size = {50} />
      <ListItem.Content>
        <ListItem.Title style = {item.read ? styles.readText : styles.unreadText}>{data.parentFullName}</ListItem.Title>
