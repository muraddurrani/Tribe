...
function MatchItem({item}) {
  ...
  return (
    <ListItem style = {styles.list} containerStyle = {styles.listItem}
      onPress = {() => navigation.navigate('CA2', {ID: item.ID, photo: data.profilePhoto, name: data.parentFullName, service: mapObjectToSentence(item.attributes[0])[0]})}
    >
      <ProfileIcon photo = {data.profilePhoto} size = {60} />
      <ListItem.Content>
        <ListItem.Title style = {{fontWeight: 'bold'}}>{data.parentFullName}</ListItem.Title>
