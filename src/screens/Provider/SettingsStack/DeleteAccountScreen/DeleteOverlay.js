...
import { removeFromSearchProviders } from '../../../../utilities/helper'
...
function DeleteOverlay({isVisible, onPress}) {
  ...
  const clearFromSearchProviders = async (id) => {
    const doc = await firestore().collection('Providers').doc(id).get()
    const responses = doc.data().Responses
    for (const obj of Object.entries(responses)) {
      removeFromSearchProviders(obj[1], obj[0])
    }
  }
