...
function index({navigation, route}) {
  ...
  const createAppointment = async (timeStart, timeEnd) => {
    ...
    if (prevAppointment) {
      await firestore().collection('Clients').doc(auth().currentUser.uid).collection('Appointments').doc(prevAppointment).delete()
      await firestore().collection('Providers').doc(ID).collection('Appointments').doc(prevAppointment).delete()
    }

    await firestore().collection('Clients').doc(auth().currentUser.uid).collection('AppointmentRequests').add({
      ID: ID,
      location: loc,
      service: service,
      timeStart,
    ...
    await firestore().collection('Providers').doc(ID).collection('PendingAppointments').doc(docRef).set({
