...
function Pending() {
  ...
  const render = ({item}) => {
    ...
    const acceptRequest = async () => {
      ...
      await firestore().collection('Clients').doc(auth().currentUser.uid).collection('Appointments').add({
        ID: item.ID,
      ...
      firestore().collection('Providers').doc(item.ID).collection('Appointments').doc(docRef).set({
      ...
      firestore().collection('Clients').doc(auth().currentUser.uid).collection('PendingAppointments').doc(item.doc).delete()
      firestore().collection('Providers').doc(item.ID).collection('AppointmentRequests').doc(item.doc).delete()
    }
  ...
  useEffect(() => {
    const unsubscribe = firestore().collection('Clients').doc(auth().currentUser.uid).collection('PendingAppointments').orderBy('timeStart', 'asc').where('timeStart', '>=', new Date()).
      onSnapshot(snapshot => {
